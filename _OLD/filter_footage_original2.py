...
# Silence OpenCV FFmpeg decoder warnings
os.environ["OPENCV_FFMPEG_LOGLEVEL"] = "-8"

# Configuration
INPUT_DIR = "./repaired_videos"
OUTPUT_DIR = "./person_clips"
PAD_SECONDS = 60          # 1 minute padding before and after
STRIDE = 5                # Sample every 5th frame (~6 fps for 30fps video)
...
def merge_intervals(intervals, max_duration):
    """Pads intervals by PAD_SECONDS and merges overlapping ranges."""
    if not intervals:
        return []
    ...
    for current_start, current_end in padded[1:]:
        ...
        if current_start <= prev_end:
            # Overlapping or adjacent: merge ranges
...
for idx, filename in enumerate(video_files, 1):
    ...
    # Group timestamps into raw start-end events
    ...
    # Cut segments using FFmpeg stream-copy (fast & lossless)
